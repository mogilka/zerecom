...
$success = false;
$name = @$_GET['name'];
if (!i18n_gallery_check_prerequisites()) {
...
<?php if (count($languages) > 0) foreach ($languages as $language) { ?>
        html += '<input type="text" class="text lang lang_<?php echo $language; ?>" name="post-item_'+i+'_title" value="" style="clear:both;float:left;width:<?php echo 383-$w; ?>px;margin-right:5px;display:none;"/>';
<?php } ?>
...
<?php if (count($languages) > 0) foreach ($languages as $language) { ?>
        html += '<textarea class="text lang lang_<?php echo $language; ?>" name="post-item_'+i+'_description" style="clear:both;float:left;width:<?php echo 588-$w; ?>px;margin-top:2px;height:14px;display:none;"></textarea>';
<?php } ?>
        html += '</td>\n';
